...
struct PlasticThreeGeneratorIntertwinerCertificate {
...
namespace plastic_three_generator_intertwiner_detail {
...
inline int generator_from_prefixes(const std::vector<long long>& left,
    ...
    throw std::runtime_error("plastic prefix defect outside {0,+a,-a}");
}
...
/** Exact three-generator comparison for the plastic substitution.
 *
 * The universal core has one state for each ordered parent-letter role (i,j).
 * Every ordered pair of parent decompositions contributes one edge labelled by
 * its exact prefix defect 0,+a,-a.  The concrete contact-boundary graph is a
 * deletion-only subgrammar: a boundary state [i,x,j] maps to (i,j), and every
 * surviving exact edge is one of the universal parent-pair witnesses.
 * Therefore, with the 0/1 incidence map P,
 *
 *   G0_B P <= P G0_U,
 *   G+_B P <= P G+_U,
 *   G-_B P <= P G-_U.
 */
inline PlasticThreeGeneratorIntertwinerCertificate
 derive_plastic_three_generator_intertwiner() {
    using namespace plastic_three_generator_intertwiner_detail;
    PlasticThreeGeneratorIntertwinerCertificate out;
    ...
    if (boundary.empty()) {
        out.obstruction = "plastic contact boundary unexpectedly empty";
        return out;
    }
    ...
    for (std::size_t s=0;s<boundary.size();++s) {
        ...
        if (n.i<0 || n.i>=3 || n.j<0 || n.j>=3) {
            ...
            out.obstruction="plastic boundary role outside alphabet";
            return out;
        }

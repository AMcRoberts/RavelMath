...
/-- Mechanically emitted: instantiates the general lemma above for
    this substitution's own incidence matrix (charpoly=x^3 + 0*x^2 + -x + -1). -/
theorem barge_diamond_instance_0 (β : ℝ) (hβ : Irrational β)
    (w : Fin 3 → ℤ) (i : Fin 3) (hwi : w i ≠ 0)
    (heig : β * (w i : ℝ) = (((!![0, 0, 1; 1, 0, 0; 1, 1, 0]).mulVec w) i : ℝ)) : False :=
  irrational_eigenvalue_has_no_integer_eigenvector !![0, 0, 1; 1, 0, 0; 1, 1, 0] w β hβ i hwi heig

/-- Mechanically emitted: instantiates the general lemma above for
    this substitution's own incidence matrix (charpoly=x^2 + -3*x + 1). -/
theorem barge_diamond_instance_1 (β : ℝ) (hβ : Irrational β)
    (w : Fin 2 → ℤ) (i : Fin 2) (hwi : w i ≠ 0)
    (heig : β * (w i : ℝ) = (((!![2, 1; 1, 1]).mulVec w) i : ℝ)) : False :=
  irrational_eigenvalue_has_no_integer_eigenvector !![2, 1; 1, 1] w β hβ i hwi heig

/-- Mechanically emitted: instantiates the general lemma above for
    this substitution's own incidence matrix (charpoly=x^2 + -x + -1). -/
theorem barge_diamond_instance_2 (β : ℝ) (hβ : Irrational β)
    (w : Fin 2 → ℤ) (i : Fin 2) (hwi : w i ≠ 0)
    (heig : β * (w i : ℝ) = (((!![1, 1; 1, 0]).mulVec w) i : ℝ)) : False :=
  irrational_eigenvalue_has_no_integer_eigenvector !![1, 1; 1, 0] w β hβ i hwi heig

/- Semantic proof graph for: barge_diamond_batch
  [0] lean.integer_eigenvector_no_witness :: n=3 M=charpoly=x^3 + 0*x^2 + -x + -1 -- instantiates irrational_eigenvalue_has_no_integer_eigenvector
  [1] lean.integer_eigenvector_no_witness :: n=2 M=charpoly=x^2 + -3*x + 1 -- instantiates irrational_eigenvalue_has_no_integer_eigenvector
  [2] lean.integer_eigenvector_no_witness :: n=2 M=charpoly=x^2 + -x + -1 -- instantiates irrational_eigenvalue_has_no_integer_eigenvector
-/

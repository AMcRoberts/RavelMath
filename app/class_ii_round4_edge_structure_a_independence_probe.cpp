// Sharper recon than class_ii_round4_pruned_states_a_independence_probe.cpp:
// that file showed the pruned-state SET is a-independent. This checks
// whether the forward-edge GRAPH STRUCTURE among the 325 raw states
...
//
// Unlike Rounds 2/3 (a-independent from a=6 on), Round 4 has ONE
// exceptional edge at a=6 specifically: state {i=0,x=(3,-3,0),j=0}
// gains one extra destination, {i=1,x=(-3,3,-2),j=0}, at a=7 that it
// does not have at a=6 (checked directly: 14 destinations at a=6, 15
// at a=7+). For a>=7 the graph is exactly a-independent (checked at
// a=7,8,9,10,12,15,20,30). a=6 sits below this project's established
// general round-family stability threshold (a>=7 is already the
// domain of the "stable" dispatcher and several other Class-II
// bounds elsewhere in this codebase), so this is very plausibly the
// same kind of boundary exception as Round 1's a=2, not a
// counterexample to the underlying pattern.
...
int main() {
    const std::vector<long long> as = {7, 8, 9, 10, 12, 15, 20, 30};
    ...
    std::printf(
        "%s\n",
        edges_all_same
            ? "ROUND4_EDGE_STRUCTURE_IS_A_INDEPENDENT_FOR_A>=7: the "
              "entire forward-edge connectivity graph over the 325 "
              "raw states (not just the pruned node set) is the "
              "literal same across a=7,8,9,10,12,15,20,30 -- a=6 is "
              "exceptional (one extra edge appears at a>=7; see file "
              "header) -- the symbolic proof reduces to checking one "
              "fixed graph for a>=7, not an affine argument per state"

// generalized_multinacci_unit_probe.cpp
//
...
template <std::size_t D>
void run_generalized_multinacci_case(std::size_t n, std::size_t m) {
    auto sigma = generalized_multinacci_rule(n, m);
    double beta = generalized_multinacci_beta(n, m);
    ...
    char name[64];
    std::snprintf(name, sizeof(name), "index=%zu D=%zu m=%zu", n - 2, n, m);
    ...
    if (rep.boundary_size == 0) {
        std::printf("  boundary graph is empty; quotient-factor check is vacuous and skipped.\n");
        return;
    }
    ...
    if (is_pure_pairing && rep.boundary_size > 0) {
        auto bpc = bp_core_charpoly(rule);
        check_exact_factor(name, bpc, gbq);
    } else if (rep.boundary_size == 0) {
        std::printf("  boundary graph is empty; quotient-factor check is vacuous and skipped.\n");
    }
}
...
void run_generalized_multinacci(std::size_t n, std::size_t m) {
    switch (n) {
        case 2: run_generalized_multinacci_case<2>(n, m); return;
        case 3: run_generalized_multinacci_case<3>(n, m); return;
        case 4: run_generalized_multinacci_case<4>(n, m); return;
        case 5: run_generalized_multinacci_case<5>(n, m); return;
        case 6: run_generalized_multinacci_case<6>(n, m); return;
        case 7: run_generalized_multinacci_case<7>(n, m); return;
        case 8: run_generalized_multinacci_case<8>(n, m); return;
        default:
...
int main(int argc, char** argv) {
    std::size_t m = 2;
    std::size_t max_index = 2;
    if (argc >= 2) m = static_cast<std::size_t>(std::stoul(argv[1]));
    if (argc >= 3) max_index = static_cast<std::size_t>(std::stoul(argv[2]));
    if (argc >= 4) {
        std::string method_arg = argv[3];
        if (method_arg == "bigfloat") g_method = mathlib::PerronCertifyMethod::TunedBigFloat;
        else if (method_arg == "exact") g_method = mathlib::PerronCertifyMethod::ExactRationalBracket;
        else { std::fprintf(stderr, "usage: %s [m] [max_index] [exact|bigfloat] [max_iters]\n", argv[0]); return 2; }
    }
    if (argc >= 5) g_max_iters = std::atoi(argv[4]);
    std::printf("=== generalized multinacci unit family m=%zu, indices 0..%zu (D=index+2) ===\n", m, max_index);
    std::printf("sigma(i)=0^m(i+1), sigma(D-1)=0; polynomial x^D-m(x+...+x^(D-1))-1\n\n");
    for (std::size_t index = 0; index <= max_index; ++index) {
        run_generalized_multinacci(index + 2, m);
    }
    return 0;
}

...
constexpr std::size_t flux_count = 55; // previous 28 + cyclic length-3 factors(27)
...
std::array<long double, flux_count> sign_flux(const ShellState& raw) {
    ...
    for (std::size_t i = 0; i < x.size(); ++i) {
        const auto j = (i + 1) % x.size();
        const auto k = (i + 2) % x.size();
        f[28 + 9 * sign_index(x[i]) + 3 * sign_index(x[j]) + sign_index(x[k])] += 1;
    }
    return f;
}

// Quick reconnaissance for the still-open Round 2 symbolic Red-
// exclusion proof: are the 123 pruned STATES (as coordinate tuples,
// not just their count/rank sizes) literally identical across
// different a, with only the forward-edge weights (M-matrix entries)
// depending on a? If so, the eventual symbolic proof only needs to
// show, for each of a small number of state categories, that a fixed
// affine-in-a forward target never lands on a survivor -- the same
// shape of argument Round 1 used, just with more categories.
//
// This does NOT attempt the symbolic proof. It only checks the
// weaker, cheaper question first: is the pruned STATE SET itself
// a-independent. Checked, not assumed.
...
int main() {
    ...
    std::printf(
        "%s\n",
        all_identical
            ? "ROUND2_PRUNED_STATE_SET_IS_A_INDEPENDENT: the 123 "
              "pruned states are the literal same coordinate set for "
              "every tested a in {6,7,8,9,10,12,15,20,30} -- only the "
              "forward-edge weights depend on a, matching Round 1's "
              "pattern; a symbolic proof only needs per-category "
              "M-matrix arguments, not a-varying state enumeration"

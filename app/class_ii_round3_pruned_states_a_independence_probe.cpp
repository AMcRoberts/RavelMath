// The Round 2 pruned-state a-independence probe
// (class_ii_round2_pruned_states_a_independence_probe.cpp), applied
// to Round 3, to find out whether all four base rounds share the
// same "fixed pruned-state set, a-varying forward weights" pattern
// or whether Round 1 (pruned coordinates affine in a, per
// class_ii_neighbor2_round1_red_symbolic.cpp) is typical or an
// outlier. Checked, not assumed.
...
int main() {
    ...
    std::printf(
        "%s\n",
        all_identical
            ? "ROUND3_PRUNED_STATE_SET_IS_A_INDEPENDENT: the 163 "
              "pruned states are the literal same coordinate set for "
              "every tested a in {6,7,8,9,10,12,15,20,30}, matching "
              "Round 2's pattern"

// item1_cycle_dynamics_check_n7.cpp
//
// n=7 (Septanacci) companion to item1_cycle_dynamics_check.cpp (see
// that file for the full derivation of the contraction recursion
// t' = (t+delta)/beta -- unchanged here, just applied one n higher).
// Purpose: Hexanacci (n=6) is where the composite-hop mechanism for
// non-dominant-SCC margin violations was first noticed (see
// docs/RESEARCH_STATUS.md, item-1 investigation); this file checks whether
// the same mechanism recurs, with the same closed-form values, at
// n=7, or whether n=6 was a coincidence. It was NOT a coincidence:
// n=7's largest non-dominant SCC (731 nodes) has worst node
// x = e_0+e_1-e_5-e_6, and <x,v> matches (v_0-v_5)+(v_1-v_6) to 6
// decimal places -- exactly the same "sum of two disjoint elementary
// hops, shifted by n-1" pattern found at n=6 (there: e_0+e_1-e_4-e_5 =
// (e_0-e_4)+(e_1-e_5)). The dominant core's own worst node is, as at
// every other n checked, x = e_0 - e_{n-1}, matching
// item1_margin_trend_check.cpp's n=7 value exactly (margin=0.004034).
//
// NOTE ON RUNTIME: this driver's corona/G_B construction is the
// expensive part -- budget several minutes of wall-clock time (the
// n=6 version already takes real time; n=7's G_B has more than double
// the nodes: 2616 vs 1221). stdio is fully buffered when redirected
// to a file (not a tty), so no output appears until the process
// actually completes or exits -- don't mistake "no output yet" for
// "hung." If running this under a tool with a short per-invocation
// wall-clock limit (as opposed to a plain interactive shell), launch
// it fully detached (e.g. `setsid nohup ./item1_cycle_dynamics_check_n7
// > out.txt 2>&1 < /dev/null &`, not just trailing `&`) and poll the
// output file across several separate waits -- a bare backgrounded
// job can get reaped when the invoking shell/session exits between
// polls, well before the computation itself is done.
...
int main() {
    ...
    double beta = 1.9919641966050352;

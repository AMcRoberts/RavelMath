...
| `include/adelic/property_f_class_ii_carry_deviation.hpp` | Exact affine deviation law: `q'=eta` for every digit, `eta'=1-eta+d` for digit deviation `d`, and every nonzero deviation exits the two-state phase strip in one step. |
| `include/adelic/property_f_class_ii_akiyama_spine.hpp` | Exact power-basis adapter from Akiyama's `k(a+beta^-1)` fundamental solutions to the negative Class-II spine residuals, including the alternating `k,a-k` affine recurrence. |
| `include/adelic/property_f_class_ii_prefix_correction.hpp` | Exact prefix-role coefficient catalogue: ordinary zero prefixes give integer contributions, while the sole exceptional role is `a+beta^(-1)`-controlled, decomposes as `a+beta^(-1)U`, and is excluded from the high letter-0 tail. |
| `include/adelic/property_f_class_ii_akiyama_window.hpp` | Computes the five-value digit-deviation window implied by transporting Akiyama's `|x_0+x_2|<=2` bound to the Class-II phase coordinate; hypothesis interface, not the transport theorem. |
| `include/adelic/property_f_class_ii_akiyama_window_census.hpp` | Applies that candidate window to realized high-tail edges, retaining internal SCC transport and rejecting nonzero bounded deviations that return to a different high SCC; collar hits become the finite cases for the symbolic proof. |
| `include/adelic/property_f_class_ii_correction_absorption.hpp` | Composes phase forcing, the realized Akiyama window, seed-only high recurrence, and the no-reentry collar into one finite-correction absorption certificate; graph-level bridge, not yet the uniform prefix/contact theorem. |
| `include/adelic/property_f_class_ii_akiyama_correction_cases.hpp` | Checks the six exact Section-4 Akiyama algebraic reductions for the bounded `(kappa_1,kappa_2)` window in the Class-II coefficient lattice; expansion admissibility remains a separate obligation. |
...
| `include/adelic/property_f_class_ii_phase_carrier.hpp` | Scalar eta/c0 odometer certificate for the phase strip, its admissible intervals, and the uniform `2a+2` spine-height mechanism. |

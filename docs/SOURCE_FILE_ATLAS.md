...
| `include/adelic/property_f_class_ii_prefix_correction.hpp` | Exact prefix-role coefficient catalogue: ordinary zero prefixes give integer contributions, while the sole exceptional role is `a+beta^(-1)`-controlled and decomposes as `a+beta^(-1)U`. |

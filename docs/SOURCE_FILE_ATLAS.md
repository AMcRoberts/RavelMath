...
| `include/adelic/quadratic_class_group.hpp` | Class number computation for imaginary quadratic fields `Q(sqrt(D))`, `D < 0`, via binary quadratic forms (Cohen Ch. 5): `qform_reduce` (unique-reduced-representative transformation), `enumerate_reduced_forms`/`quadratic_class_number` (direct search, `O(sqrt(-D))` bound). Task #4 of the class-field-tower initiative, deliberately scoped to the imaginary-quadratic case rather than general-degree-n class groups (no LLL/Buchmann needed). `tests/quadratic_class_group_test.cpp` checks all nine Heegner numbers against an independently-constructed principal form, several tabulated `h>1` cases, and cross-checks `qform_reduce` against `enumerate_reduced_forms`'s independent search (this caught a real off-by-one bug in the reduction step). Group *structure* (Gauss composition) not yet built -- this file gives the class number/underlying set only. |
